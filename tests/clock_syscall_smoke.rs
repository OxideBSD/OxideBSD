...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "clock_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before clock-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_CLOCK_GETTIME/SYS_CLOCK_GETRES/SYS_CLOCK_SETTIME/SYS_CLOCK_NANOSLEEP/
    // SYS_NANOSLEEP/SYS_SETITIMER/SYS_GETITIMER/SYS_TIMER_* -- this test's entire subject.
    ...
    const CLOCK_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("CLOCK_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "clock_syscall_smoke: spawning clock-syscall-smoke as pid 1 ({} byte ELF)",
        CLOCK_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(CLOCK_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn clock-syscall-smoke: {e:?}"));
