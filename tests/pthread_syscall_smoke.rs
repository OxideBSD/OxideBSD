...
//! needs), `signal` (`SYS_SIGPROCMASK` -- musl's `pthread_create`'s `start()` trampoline
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "pthread_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before pthread-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_KILL/SYS_SIGACTION/SYS_SIGPROCMASK/SYS_SIGRETURN -- real musl pthread_create's
    // own start() trampoline unconditionally calls rt_sigprocmask for every new thread.
    ...
    // Serves /pthread-smoke.elf for the real fork+execve below.
    ...
    let pid1 = oxidebsd::process::spawn(PTHREAD_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn pthread-syscall-smoke: {e:?}"));
