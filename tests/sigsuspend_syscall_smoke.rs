...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "sigsuspend_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before sigsuspend-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_KILL/SYS_SIGACTION/SYS_SIGPROCMASK/SYS_SIGPENDING/SYS_SIGSUSPEND -- this
    // test's entire subject.
    ...
    const SIGSUSPEND_SYSCALL_SMOKE_ELF: &[u8] =
        include_bytes!(env!("SIGSUSPEND_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "sigsuspend_syscall_smoke: spawning sigsuspend-syscall-smoke as pid 1 ({} byte ELF)",
        SIGSUSPEND_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(SIGSUSPEND_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn sigsuspend-syscall-smoke: {e:?}"));
