...
//! `sched_setscheduler/19-1,20-1,21-1.c`/`sched_yield/2-1.c` files.
//!
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "sched_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before sched-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_SETUID and the whole SYS_SCHED_* family -- this test's entire subject.
    ...
    const SCHED_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("SCHED_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "sched_syscall_smoke: spawning sched-syscall-smoke as pid 1 ({} byte ELF)",
        SCHED_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(SCHED_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn sched-syscall-smoke: {e:?}"));
