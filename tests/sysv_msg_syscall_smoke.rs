...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "sysv_msg_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before sysv-msg-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_MSGGET/SYS_MSGSND/SYS_MSGRCV/SYS_MSGCTL -- this test's entire subject.
    ...
    const SYSV_MSG_SYSCALL_SMOKE_ELF: &[u8] =
        include_bytes!(env!("SYSV_MSG_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "sysv_msg_syscall_smoke: spawning sysv-msg-syscall-smoke as pid 1 ({} byte ELF)",
        SYSV_MSG_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(SYSV_MSG_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn sysv-msg-syscall-smoke: {e:?}"));
