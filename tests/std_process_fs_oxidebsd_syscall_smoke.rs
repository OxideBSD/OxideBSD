...
//! `std_hello_oxidebsd_syscall_smoke.rs` already covers).
//!
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "std_process_fs_oxidebsd_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    const STD_PROCESS_FS_OXIDEBSD_SYSCALL_SMOKE_ELF: &[u8] =
        include_bytes!(env!("STD_PROCESS_FS_OXIDEBSD_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "std_process_fs_oxidebsd_syscall_smoke: spawning std-process-fs-oxidebsd-syscall-smoke as pid 1 ({} byte ELF)",
        STD_PROCESS_FS_OXIDEBSD_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(STD_PROCESS_FS_OXIDEBSD_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn std-process-fs-oxidebsd-syscall-smoke: {e:?}"));
