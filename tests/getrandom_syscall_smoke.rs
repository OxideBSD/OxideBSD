...
//! `posix_compat` (`SYS_GETRANDOM`), then spawns `regress/getrandom-syscall-smoke/` as pid 1 --
//! see that crate's own module doc comment for the full four-part scenario (a real 32-byte
//! request, two consecutive requests differing, `len == 0` as a no-op, and
//! `GRND_NONBLOCK`/`GRND_RANDOM`/an-invalid-flag's `EINVAL`).
//!
...
/// Must match `regress/getrandom-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "getrandom_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before getrandom-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_GETRANDOM -- this test's entire subject.
    ...
    const GETRANDOM_SYSCALL_SMOKE_ELF: &[u8] =
        include_bytes!(env!("GETRANDOM_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "getrandom_syscall_smoke: spawning getrandom-syscall-smoke as pid 1 ({} byte ELF)",
        GETRANDOM_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(GETRANDOM_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn getrandom-syscall-smoke: {e:?}"));
