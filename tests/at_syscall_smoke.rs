//! Boots the full kernel with the same module set as `tests/pthread_syscall_smoke.rs` (native_abi,
//! signal, posix_compat, oxfs) and spawns `regress/at-syscall-smoke/` as pid 1, which runs
//! `/at-smoke.elf` -- real musl-linked coverage of the whole `*at()` family (see
//! `regress/at-smoke/main.c`). Same `SYS_TEST_EXIT` convention as `tests/fork_wait.rs`.
#![no_std]
#![no_main]
...
/// Must match `regress/at-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "at_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before at-syscall-smoke, below, is spawned.
    ...
    // Serves /at-smoke.elf and every *at() call it makes.
    ...
    const AT_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("AT_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "at_syscall_smoke: spawning at-syscall-smoke as pid 1 ({} byte ELF)",
        AT_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(AT_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn at-syscall-smoke: {e:?}"));
