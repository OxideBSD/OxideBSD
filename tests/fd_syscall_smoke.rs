//! Boots the kernel with `tests/ppoll_syscall_smoke.rs`'s module set and spawns
//! `regress/fd-syscall-smoke/` as pid 1, which runs `/fd-smoke.elf` -- lowest-free fd numbering
//! and FIFOs through real musl (see `regress/fd-smoke/main.c`).
#![no_std]
#![no_main]
...
/// Must match `regress/fd-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "fd_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before fd-syscall-smoke, below, is spawned.
    ...
    // Registers SYS_POLL (the FIFO readiness checks).
    ...
    const FD_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("FD_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "fd_syscall_smoke: spawning fd-syscall-smoke as pid 1 ({} byte ELF)",
        FD_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(FD_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn fd-syscall-smoke: {e:?}"));
