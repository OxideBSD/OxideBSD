//! Boots the kernel and spawns `regress/sh-syscall-smoke/` as pid 1: OxideBSD's own shell
//! (`/bin/sh`, `/sbin/init_sh`) running the differential corpus on target (INIT_SH.md §8.3).
#![no_std]
#![no_main]
...
/// Must match `regress/sh-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "sh_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before sh-syscall-smoke, below, is spawned.
    ...
    const SH_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("SH_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "sh_syscall_smoke: spawning sh-syscall-smoke as pid 1 ({} byte ELF)",
        SH_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(SH_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn sh-syscall-smoke: {e:?}"));
