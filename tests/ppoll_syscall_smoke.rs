//! Boots the kernel with `tests/at_syscall_smoke.rs`'s module set plus `net` (which registers
//! `SYS_PPOLL`) and spawns `regress/ppoll-syscall-smoke/` as pid 1, which runs `/ppoll-smoke.elf`
//! -- real musl `ppoll()` coverage, mask semantics included (see `regress/ppoll-smoke/main.c`).
#![no_std]
#![no_main]
...
/// Must match `regress/ppoll-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "ppoll_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before ppoll-syscall-smoke, below, is spawned.
    ...
    const PPOLL_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("PPOLL_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "ppoll_syscall_smoke: spawning ppoll-syscall-smoke as pid 1 ({} byte ELF)",
        PPOLL_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(PPOLL_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn ppoll-syscall-smoke: {e:?}"));
