...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "needs_syscall2_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before needs-syscall2-smoke, below, is spawned.
    ...
    // Populates SYS_SETUID (for the non-root chroot EPERM check) and this pass's new
    // SYS_GETRUSAGE.
    ...
    // Populates SYS_OPEN/SYS_CLOSE/SYS_MKDIR/SYS_CHDIR/SYS_GETCWD/SYS_STAT/SYS_UNLINK, plus this
    // pass's new SYS_LINK/SYS_MKNOD/SYS_CHROOT -- this test's whole filesystem-side scenario.
    ...
    const NEEDS_SYSCALL2_SMOKE_ELF: &[u8] = include_bytes!(env!("NEEDS_SYSCALL2_SMOKE_ELF_PATH"));
    serial_println!(
        "needs_syscall2_smoke: spawning needs-syscall2-smoke as pid 1 ({} byte ELF)",
        NEEDS_SYSCALL2_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(NEEDS_SYSCALL2_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn needs-syscall2-smoke: {e:?}"));
