...
//! registration path) and `signal` (`SYS_KILL`/`SYS_SIGACTION`/`SYS_SIGPROCMASK`/`SYS_SIGPENDING`/
//! `SYS_SIGTIMEDWAIT`/`SYS_SIGQUEUE`), then spawns `regress/rt-signal-syscall-smoke/` as pid 1 --
//! see that crate's own module doc comment for the full five-part real-time-signal-queuing
//! scenario.
//!
...
/// Must match `regress/rt-signal-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "rt_signal_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before rt-signal-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_KILL/SYS_SIGACTION/SYS_SIGPROCMASK/SYS_SIGPENDING/SYS_SIGTIMEDWAIT/
    // SYS_SIGQUEUE -- this test's entire subject.
    ...
    const RT_SIGNAL_SYSCALL_SMOKE_ELF: &[u8] =
        include_bytes!(env!("RT_SIGNAL_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "rt_signal_syscall_smoke: spawning rt-signal-syscall-smoke as pid 1 ({} byte ELF)",
        RT_SIGNAL_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(RT_SIGNAL_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn rt-signal-syscall-smoke: {e:?}"));
