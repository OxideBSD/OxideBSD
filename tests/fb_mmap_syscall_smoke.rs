...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "fb_mmap_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // SYS_MUNMAP -- must load before fb-mmap-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_IOCTL -- backs this test's own FBIOGET_OXIDEBSD request.
    ...
    // Populates SYS_OPEN/SYS_CLOSE, plus the real /dev/fb0 device inode this test's own part 1
    // depends on.
    ...
    const FB_MMAP_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("FB_MMAP_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "fb_mmap_syscall_smoke: spawning fb-mmap-syscall-smoke as pid 1 ({} byte ELF)",
        FB_MMAP_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(FB_MMAP_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn fb-mmap-syscall-smoke: {e:?}"));
