...
//! `posix_compat` (`SYS_GET_KEYEVENT = 558`), then spawns `regress/keyevent-syscall-smoke/` as
...
/// Must match `regress/keyevent-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
