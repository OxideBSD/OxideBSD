...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "sysinfo_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before sysinfo-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_SYSINFO -- this test's entire subject.
    ...
    const SYSINFO_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("SYSINFO_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "sysinfo_syscall_smoke: spawning sysinfo-syscall-smoke as pid 1 ({} byte ELF)",
        SYSINFO_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(SYSINFO_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn sysinfo-syscall-smoke: {e:?}"));
