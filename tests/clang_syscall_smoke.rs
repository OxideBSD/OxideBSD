...
//! mmap/brk/set_fs_base -- everything musl's own runtime needs for a real `clang` invocation:
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "clang_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    const CLANG_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("CLANG_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "clang_syscall_smoke: spawning clang-syscall-smoke as pid 1 ({} byte ELF)",
        CLANG_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(CLANG_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn clang-syscall-smoke: {e:?}"));
