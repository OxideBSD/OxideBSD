...
//! registration path) and `posix_compat` (`SYS_SEMGET`...`SYS_SEMTIMEDOP`), then spawns
//! `regress/sysv-sem-syscall-smoke/` as pid 1 -- see that crate's own module doc comment for the
//! full seven-part real-SysV-semaphore scenario.
//!
...
/// Must match `regress/sysv-sem-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "sysv_sem_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before sysv-sem-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_SEMGET/SYS_SEMOP/SYS_SEMCTL/SYS_SEMTIMEDOP -- this test's entire subject.
    ...
    const SYSV_SEM_SYSCALL_SMOKE_ELF: &[u8] =
        include_bytes!(env!("SYSV_SEM_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "sysv_sem_syscall_smoke: spawning sysv-sem-syscall-smoke as pid 1 ({} byte ELF)",
        SYSV_SEM_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(SYSV_SEM_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn sysv-sem-syscall-smoke: {e:?}"));
