...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "clone_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // must load before clone-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_FUTEX -- the real join primitive this test's own child/parent handshake uses.
    ...
    const CLONE_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("CLONE_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "clone_syscall_smoke: spawning clone-syscall-smoke as pid 1 ({} byte ELF)",
        CLONE_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(CLONE_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn clone-syscall-smoke: {e:?}"));
