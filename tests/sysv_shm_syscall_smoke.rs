...
//! registration path) and `posix_compat` (`SYS_SHMGET`...`SYS_SHMDT`), then spawns
//! `regress/sysv-shm-syscall-smoke/` as pid 1 -- see that crate's own module doc comment for the
//! full six-part real-SysV-shared-memory scenario.
//!
...
/// Must match `regress/sysv-shm-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "sysv_shm_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before sysv-shm-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_SHMGET/SYS_SHMAT/SYS_SHMCTL/SYS_SHMDT -- this test's entire subject.
    ...
    const SYSV_SHM_SYSCALL_SMOKE_ELF: &[u8] =
        include_bytes!(env!("SYSV_SHM_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "sysv_shm_syscall_smoke: spawning sysv-shm-syscall-smoke as pid 1 ({} byte ELF)",
        SYSV_SHM_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(SYSV_SHM_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn sysv-shm-syscall-smoke: {e:?}"));
