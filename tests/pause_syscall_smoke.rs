...
//! registration path) and `signal` (`SYS_SIGACTION`/`SYS_KILL`/`SYS_PAUSE`), then spawns
//! `regress/pause-syscall-smoke/` as pid 1 -- see that crate's own module doc comment for the
//! full fork+pause+wake+reap scenario.
//!
...
/// Must match `regress/pause-syscall-smoke/src/main.rs`'s own `SYS_TEST_EXIT` constant -- no
...
extern "C" fn test_exit_handler(code: u64, _arg1: u64, _arg2: u64, _arg3: u64) -> i64 {
    serial_println!(
        "pause_syscall_smoke: child reported {}",
...
fn main(boot_info: &'static BootInfo) -> ! {
    ...
    // before pause-syscall-smoke, below, is spawned.
    ...
    // Populates SYS_KILL/SYS_SIGACTION/SYS_PAUSE -- this test's entire subject.
    ...
    const PAUSE_SYSCALL_SMOKE_ELF: &[u8] = include_bytes!(env!("PAUSE_SYSCALL_SMOKE_ELF_PATH"));
    serial_println!(
        "pause_syscall_smoke: spawning pause-syscall-smoke as pid 1 ({} byte ELF)",
        PAUSE_SYSCALL_SMOKE_ELF.len()
    );
    let pid1 = oxidebsd::process::spawn(PAUSE_SYSCALL_SMOKE_ELF, None)
        .unwrap_or_else(|e| panic!("failed to spawn pause-syscall-smoke: {e:?}"));
