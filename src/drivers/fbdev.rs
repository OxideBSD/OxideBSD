...
/// The wire format both `modules/oxfs` (seeding a `/dev/fb0` device inode's backing info) and
...
/// Module-callable wrapper (see `src/module.rs`'s hand-curated symbol table) -- writes the real
