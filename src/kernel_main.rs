...
//! pid 1), and hands off to the scheduler. Lives in the library (not `src/main.rs`) so **every**
//! boot-path entry point can call the exact same code, not just the Limine one: `src/main.rs`'s
//! `kernel_main` (Limine) and the Multiboot2 path's own dedicated entry crate
//! (`smoke/multiboot2-kernel/`, see CLAUDE.md's Multiboot2 section) both just call
//! [`run_real_system`] directly with whatever `&'static BootInfo` their own entry macro produced.
//!
//! **Why this can't just live in `src/main.rs` and be reused via a second `[[bin]]`-style crate
//! pointed at that same source file** (the same trick `smoke/multiboot2-boot-smoke` uses for its
...
///
/// Before that, loads the `hello` kernel module (`modules/hello/`) via `module::load` — see
