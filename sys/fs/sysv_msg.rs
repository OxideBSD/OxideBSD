...
//!
//! **A fundamentally different lifecycle from `sys/fs/mqueue.rs`'s POSIX message queues, despite
...
//! type, flag`) and this ABI only carries 4 -- `external/mit/musl/src/ipc/msgrcv.c` is patched
...
//! `IPC_CMD(cmd)` macro does this unconditionally on every call (`external/mit/musl/src/ipc/
...
/// `uid`/`gid` (first match wins) -- same shape `sys/modules/oxfs`'s own `check_access` already
