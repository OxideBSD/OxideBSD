...
//! kernel, never directly into another module (see `sys/module.rs`'s doc comment). Concretely:
//! `modules/fat32/`'s open files register their read/write/close callbacks here; `sys/syscall.rs`'s
...
/// time) so it reflects a fd's *current* state — e.g. `sys/modules/oxfs`'s own `OpenFile::Write`
...
/// kind except `sys/modules/oxfs`'s own file-backed `OpenFile` variants, via `no_content_id` below).
...
/// kind except `sys/modules/oxfs`'s own file-backed `OpenFile` variants -- every other kind (pipes,
...
/// to remember this for its own write-positioning logic, see `sys/modules/oxfs`'s `OpenFile::Write::
...
/// (`external/mit/musl/src/aio/aio.c`) queries exactly this bit once per fd (`q->append =
...
/// (`default_is_append` below, `false`) is correct for every fd kind except `sys/modules/oxfs`'s own
...
/// `no_fb_geometry` below) -- only `sys/modules/oxfs`'s own `OpenFile::Framebuffer` fds ever register
/// a real one.
...
/// reads it back via `fcntl(fd, F_GETFD)`). Set by `sys/modules/oxfs`'s own `open()` path when the
...
/// `sys/module.rs`'s kernel API table), since `set_cloexec` above needs the caller's real pid,
/// which modules can't resolve themselves (only the kernel exports `scheduler::current_pid`
/// indirectly, through functions like this one). `sys/modules/oxfs`'s own `oxfs_open` is the one real
...
/// `oxidebsd_set_fd_access_mode` already establishes. `sys/modules/oxfs` is the one real caller,
...
/// `SYS_DUP2`'s real logic (`sys/syscall.rs`'s `sys_dup2` is a thin wrapper over this), scoped to
...
/// not designed in up front: `sys/modules/oxfs`'s `oxfs_read` unconditionally fails (`-EBADF`) for a
...
/// currently open against it, so there's nothing to key a per-fd table by. Only `sys/modules/oxfs`
...
/// accessors (never happens once `sys/modules/oxfs` has loaded, which it always has by the time any
...
/// exactly `len` bytes, matching `sys/modules/oxfs`'s own one-shot write model (see that module's
...
/// FFI wrapper over `real_fd_of` for modules that own their own fd-keyed state (`sys/modules/oxfs`'s
...
/// symbol table (see `sys/module.rs`). `SYS_FSTAT`'s handler is the first case that needs this:
...
/// own doc comment in `sys/syscall.rs`).
...
/// identical "loop until -1" shape, for `sys/modules/oxfs`'s own `/proc/<pid>/fd/` directory listing.
