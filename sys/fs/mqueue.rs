...
//! real `mq_open(3)`'s own raw syscall (`external/mit/musl/src/mq/mq_open.c`) already fills all 4
...
//! `external/mit/musl/src/mq/mq_timedsend.c`/`mq_timedreceive.c` are patched (see those files' own
...
//! load-bearing.
//!
//! **`mq_close` isn't its own syscall at all** -- real `mq_close(3)` (`external/mit/musl/src/mq/
...
//! permission check, see `sys/process/signals.rs`'s `has_signal_permission` -- irrelevant here
...
//! (`external/mit/musl/src/mq/mq_notify.c`) never even issues the raw syscall for that case (it's
...
/// format (`external/mit/musl/src/mq/mq_unlink.c` is a bare `__syscall(SYS_mq_unlink, name)`, no
...
/// format (`external/mit/musl/src/mq/mq_notify.c` issues this raw syscall directly whenever
...
/// around this one syscall, see `external/mit/musl/src/mq/mq_{get,set}attr.c`). Real semantics:
