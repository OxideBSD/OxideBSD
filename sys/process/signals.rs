...
/// Real Linux `force_sig()`-style semantics for a signal generated by an actual, synchronous
/// hardware fault (`interrupts::page_fault_handler`/`general_protection_fault_handler`'s own
/// ring-3 fault-to-signal self-`kill` -- see either one's own doc comment) -- **not** the same
/// thing as an ordinary, asynchronously-delivered `kill(2)`/`sigqueue(2)` signal, which must
/// respect the target's own `sigprocmask`. Real POSIX documents blocking a signal that is then
/// synchronously generated (by the process's own faulting instruction, not `kill`/`sigqueue`/
/// `raise`) as undefined behavior; real Linux resolves it by force-unblocking the signal right
/// here (`force_sig_info()`'s own `sigdelset(&t->blocked, sig)`, before ever queuing it) rather
/// than letting the block silently swallow a fault that can't just be "resumed past."
///
/// **Found live** via `pthread_kill/6-1.c`: real, unmodified musl's own `pthread_kill()`/
/// `pthread_cancel()` call `__block_all_sigs()` internally, masking every signal (including
/// `SIGSEGV`) for a short internal critical section. A real hardware fault occurring inside that
/// exact window (this test's own `pthread_kill(already_joined_tid, 0)`, dereferencing a `tid`
/// pointer into memory `pthread_join()`'s own real stack `munmap()` already freed) used to queue a
/// pending bit that `deliver_pending_signal`'s own `take_deliverable_signal` correctly refused to
/// act on -- it's blocked, by the normal rules -- leaving `process::fault_trampoline`'s
/// `SYS_FAULT_PUMP` dispatch with genuinely nothing to deliver. That trampoline's own doc comment
/// documents its `ud2` tail as unreachable specifically *because* `deliver_pending_signal` was
/// assumed to always find something -- this was the one real way to violate that assumption,
/// surfacing as an unbounded, whole-VM-halting `EXCEPTION: INVALID OPCODE` instead of the normal
/// per-process `SIGSEGV` termination every other fault-to-signal path already produces correctly.
///
/// Only ever clears the *one* bit being force-delivered -- every other signal `pthread_kill()`'s
/// own critical section blocked stays blocked, unaffected, matching real Linux's own narrow scope
/// for this fix.
pub(crate) fn force_fault_signal(pid: Pid, sig: u64) {
    {
...
/// `sys_sigaction`'s own doc comment in `sys/syscall/ffi.rs` for the matching fix on that call
...
/// this exact syscall's own dispatch tail (`sys/syscall.rs`'s `deliver_pending_signal`), since the
...
/// `si_value` (`external/mit/musl/include/signal.h`'s `__sigchld`/`si_value` union), so no new
...
/// this exact syscall's own dispatch tail (`sys/syscall/mod.rs`'s `deliver_pending_signal`, called
...
/// `deliver_pending_signal` (`sys/syscall.rs`, called right after this returns, still within the
...
/// `SIGKILL`/`SIGSTOP` — by
/// `sys/syscall.rs`'s `sys_sigaction` before this is reached). Reads/writes a real musl
...
/// Called once, at the tail of every completed syscall (`sys/syscall.rs`'s
...
///
/// `chained` distinguishes `do_sigreturn`'s own synchronous chaining call (`chained = true`) from
/// every other call site (a completed syscall's own tail, or the fault trampoline -- always
/// `chained = false`) -- see `Process::cascade_budget`'s own doc comment for why this distinction
/// is load-bearing, not cosmetic.
///
/// **Only a genuinely fresh episode (`!chained` *and* `signal_stack` is empty -- no handler is
/// currently active anywhere on this thread) (re)snapshots the budget.** This matters because an
/// *ordinary* syscall issued from *within* an already-active handler reaches this same function
/// through the exact same `!chained` call site (musl's own `pthread_cancel()`'s `SIGCANCEL`
/// resend does exactly this: `cancel_handler` issues a raw `tkill` syscall on itself) -- if that
/// nested call resnapshotted too, it would silently overwrite the *outer* episode's own budget
/// with a snapshot taken mid-handler, defeating the whole point.
///
/// **Only a chained call is budget-*gated*.** A non-chained call (fresh *or* nested) always
/// delivers freely: real forward progress genuinely happened just before it reached this function
/// (either the original program's own code, or -- for a nested call -- the active handler's own
/// code around its syscall), so admitting a newly-pending signal there is real, hardware-accurate
/// behavior, not the "redelivered without ever resuming" bug this budget exists to close. A
/// chained call, by contrast, may only consume an instance the cascade's *outermost* snapshot
/// already accounted for -- once exhausted for the lowest-numbered currently-deliverable signal,
/// this returns `None` even though something may genuinely still be pending, so `do_sigreturn`
/// stops chaining and actually resumes the restored frame instead, exactly matching what real
/// hardware would do (the next genuine kernel entry gets its own fresh, fair snapshot).
///
/// Every actual delivery (chained or not) decrements the budget for that exact signal number
/// regardless of whether this call was gated by it -- otherwise a *fresh* delivery of signal X
/// would leave X's own budget slot untouched, and a later chained call re-encountering signal X
/// (now a brand-new instance generated by X's own handler, not the original pre-cascade one)
/// would wrongly still find it "budgeted".
pub(crate) fn take_deliverable_signal(pid: Pid, chained: bool) -> Option<SignalDelivery> {
    let mut table = PROCESS_TABLE.lock();
    let proc = table.get_mut(&pid)?;
    if !chained && proc.signal_stack.is_empty() {
        let deliverable_now = proc.pending_signals & !proc.blocked_signals;
        proc.cascade_budget = Some(CascadeBudget {
            standard: deliverable_now,
            rt: core::array::from_fn(|i| {
                let sig = SIGRTMIN + i as u64;
                if deliverable_now & (1 << (sig - 1)) != 0 {
                    proc.rt_queue[i].len() as u16
                } else {
                    0
                }
            }),
    ...
    loop {
        ...
        if deliverable == 0 {
            if chained {
                proc.cascade_budget = None;
            }
            return None;
        }
        let signum = deliverable.trailing_zeros() as u64 + 1;
        if chained {
            let budgeted = proc.cascade_budget.as_ref().is_some_and(|b| {
                if signum >= SIGRTMIN {
                    b.rt[(signum - SIGRTMIN) as usize] > 0
                } else {
                    b.standard & (1 << (signum - 1)) != 0
                }
            });
            if !budgeted {
                proc.cascade_budget = None;
        ...
        let siginfo = if signum >= SIGRTMIN {
            ...
            if let Some(budget) = proc.cascade_budget.as_mut() {
                budget.rt[(signum - SIGRTMIN) as usize] -= 1;
            }
            info
        } else {
            proc.pending_signals &= !(1 << (signum - 1));
            if let Some(budget) = proc.cascade_budget.as_mut() {
                budget.standard &= !(1 << (signum - 1));
            }
...
/// `sigreturn`'s real logic (`sys/syscall.rs`'s `do_sigreturn` — kept here, not there, since it
...
/// (`external/mit/musl/src/signal/sigqueue.c`'s own `syscall(SYS_rt_sigqueueinfo, pid, sig, &si)`),
...
/// path (`sys/syscall/mod.rs`'s `deliver_pending_signal`) now populates its own `si_pid`/`si_uid`/
/// `si_value` from.
///
