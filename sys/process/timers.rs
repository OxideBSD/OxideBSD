...
/// `TIMER_HZ` ground truth, the same one `sys/syscall.rs`'s `sys_clock_gettime` already uses for
...
/// what musl's own `setitimer()`/`getitimer()` (`external/mit/musl/src/signal/{set,get}itimer.c`)
...
/// `external/mit/musl/src/unistd/alarm.c` — so this one syscall backs both real libc entry points).
///
...
/// Real, architecture-generic `clockid_t` values -- duplicated from `sys/syscall/ffi.rs`'s own
...
/// Real Linux `TIMER_ABSTIME` (`external/mit/musl/include/time.h`) -- the one `timer_settime(2)`
/// `flags` bit this ABI understands; any other bit is `EINVAL`.
const TIMER_ABSTIME: u64 = 1;

/// musl's own `struct ksigevent` (`external/mit/musl/src/time/timer_create.c`) -- the real,
...
/// function's own smoke test (`regress/clock-syscall-smoke`'s part 7): the latter reads the real
...
/// `time_t` this size, see `external/mit/musl/arch/x86_64/bits/syscall.h.in`'s own comment on the
...
/// `__clock_nanosleep` (`external/mit/musl/src/time/clock_nanosleep.c`) only ever issues this
