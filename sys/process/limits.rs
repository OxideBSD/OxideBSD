...
/// "error"); musl's own `getpriority()` wrapper un-shifts this client-side (`external/mit/musl/
...
/// (`external/mit/musl/src/sched/sched_setscheduler.c`) is a pure passthrough of that raw value --
...
/// (`external/mit/musl/src/thread/pthread_setschedparam.c`) computes
...
/// `ENOSYS` (`external/mit/musl/src/sched/sched_setparam.c`), unlike `sched_getparam`/
...
/// `external/mit/musl/arch/x86_64/bits/syscall.h.in`, no kernel handler existed until now). Reports
...
/// see `external/mit/musl/src/sched/affinity.c`'s own `do_getaffinity`) is the number of bytes
...
/// (`external/mit/musl/src/thread/__timedwait.c`'s `sem_timedwait`/`__timedwait_cp` folds any
...
/// through the test's own source (`external/gpl2/posixtestsuite/conformance/interfaces/sem_wait/
...
/// mapping, see `external/mit/musl/src/thread/sem_open.c`, which `sem_init`s the new semaphore with
...
fn futex_key(pid: Pid, addr: u64, private: bool) -> Result<(Pid, u64), u64> {
    ...
    // SAFETY: `do_futex`'s only caller (`sys/syscall/ffi.rs`) always passes
