...
fn rtl8139_irq_handler() {
    ...
    // cycle count it happened to land) -- feeds `sys/random.rs`'s persistent entropy pool. No
...
/// guarantee across separate calls (see its own doc comment in `sys/memory.rs`) -- in practice,
