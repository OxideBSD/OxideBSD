...
//! needs beyond what `sys/modules/native_abi/` (the small, BSD-authentic core: `exit`/`read`/`write`/
...
//! Linux's own `__NR_setsid` value (confirmed against `external/mit/musl/arch/x86_64/bits/
//! syscall.h.in` directly), reused verbatim because `external/mit/musl/src/unistd/setsid.c` is a
...
//! are real Linux/generic values too — see `sys/syscall.rs`'s `sys_ioctl` for what's actually
...
//! continues the sequence past `sys/modules/oxfs`'s `SYS_MKDIR = 136`. Real logic (`sys/syscall.rs`'s
...
//! analysis" -- the "uid/passwd-db model" gap) continue the sequence past `sys/modules/clock`'s own
...
//! already independently claimed by this ABI's own `SYS_KILL` (see `sys/modules/signal/`), a real
//! collision found live tracing why `su`'s own `initgroups()` call was dying instead of degrading
//! gracefully (see `sys/syscall.rs`'s `ENOSYS` doc comment for the other half of that story). Real
...
//! needed beyond the number remap already in `bits/syscall.h.in`. Real logic (`sys/syscall/
//! ffi.rs`'s `sys_getrandom`) is kernel-resident and delegates straight to `sys/random.rs`'s
...
//! `(info_ptr)` wire format, no argument-convention patch needed. Real logic (`sys/syscall/
...
unsafe extern "C" {
    ...
    fn oxidebsd_sys_futex_requeue(addr: u64, addr2: u64, nr_wake: u64, nr_requeue: u64) -> i64;
    fn oxidebsd_sys_mlock(addr: u64, len: u64) -> i64;
    fn oxidebsd_sys_munlock(addr: u64, len: u64) -> i64;
    fn oxidebsd_sys_mlockall(flags: u64) -> i64;
    fn oxidebsd_sys_munlockall() -> i64;
...
/// exactly (see `sys/syscall.rs`'s own doc comments on `sys_pipe`/`sys_dup2` for why neither needed
...
/// `sys/syscall.rs` for why this is the one syscall in this whole module that needed neither an
...
/// `sys_getsid`'s own doc comment in `sys/syscall.rs`. Continues the invented sequence right past
/// `sys/modules/oxfs`'s own `SYS_UMOUNT2 = 176`.
const SYS_GETSID: u64 = 177;

/// `SYS_PRLIMIT64 = 478` through `SYS_REBOOT = 486` are this ABI's process-attribute half of the
/// NEEDS_SYSCALL gap-table pass (`sys/modules/oxfs`'s own `SYS_FSYNC = 471` through `SYS_FSTATFS =
...
/// still-inert Linux syscalls sharing those same low numbers (see `external/mit/musl/arch/
...
/// `SYS_SENDTO` -- `external/mit/musl/arch/x86_64/bits/syscall.h.in` already remapped
...
/// this real value). See `process::do_sched_getaffinity`'s own doc comment in `sys/process.rs` for
...
/// `process::do_futex`'s own doc comment in `sys/process/limits.rs` for why this is a real, honest
/// failure stub (not real futex support) and the live `sem_wait/7-1.c` infinite-busy-loop it closes.
const SYS_FUTEX: u64 = 202;
/// OxideBSD's own invention (`557`, continuing right past `SYS_EXIT_GROUP=556`, the current
/// highest assigned number as of this addition). See `process::do_futex_requeue`'s own doc comment
/// in `sys/process/limits.rs` for why real `FUTEX_REQUEUE` needed a dedicated syscall rather than
...
/// `SYS_SET_TID_ADDRESS`/`SYS_FCNTL`/`SYS_SHUTDOWN` — `external/mit/musl/arch/x86_64/bits/
/// syscall.h.in` (the `oxidebsd` branch) was already patched in a prior collision-avoidance sweep
/// to move them here (`509`-`512`), continuing the same `493+` block `docs/
/// MISSING_POSIX_SYSCALLS.md`'s own collision table documents — but no kernel-side handler was ever
/// registered at the new numbers, so every real caller (including the POSIX conformance pilot's own
/// `mlock`/`munlock`/`mlockall`/`munlockall` test files) hit a clean `ENOSYS` until now. See
/// `process::mm::do_mlock`'s own doc comment for why these are honest, range-checked no-op
/// successes rather than real memory locking.
const SYS_MLOCK: u64 = 509;
const SYS_MUNLOCK: u64 = 510;
...
/// checks for an error at all). See `sys/process.rs`'s `do_umask`/`Process::umask` for the real
/// per-process, stored-not-enforced semantics this backs.
const SYS_UMASK: u64 = 487;
/// Invented -- continues past `sys/modules/oxfs`'s own `SYS_CHROOT = 490` (the highest number assigned
...
/// `SYS_PRLIMIT64`/`SYS_SETPRIORITY` above, not `sys/modules/oxfs` (no filesystem state involved). See
/// `sys/syscall.rs`'s own `sys_getrusage`/`RawRusage` for the real (honestly all-zero) logic.
...
/// process-resource-adjacent placement as `SYS_GETRUSAGE` just above (`sys/syscall/ffi.rs`'s
...
/// free). Real logic (`sys/syscall/ffi.rs`'s `sys_getrandom`, delegating to `sys/random.rs`'s
...
/// see `sys/fs/mqueue.rs`'s own doc comment for the real logic (`crate::fs::mqueue`, kernel-
...
/// queues, see `sys/fs/sysv_msg.rs`'s own doc comment (`crate::fs::sysv_msg`, kernel-resident
...
/// `sys/fs/sysv_sem.rs`'s own doc comment for the full design.
...
/// thing out. See `sys/fs/sysv_shm.rs`'s own doc comment for the full design.
...
/// `external/mit/musl/src/unistd/pipe2.c`'s own `ENOSYS`-triggered fallback (plain `pipe()` +
...
/// Real `reboot(int type)`'s musl-side call site (`external/mit/musl/src/linux/reboot.c`) passes
...
extern "C" fn handle_futex_requeue(addr: u64, addr2: u64, nr_wake: u64, nr_requeue: u64) -> i64 {
    unsafe { oxidebsd_sys_futex_requeue(addr, addr2, nr_wake, nr_requeue) }
}

extern "C" fn handle_mlock(addr: u64, len: u64, _a2: u64, _a3: u64) -> i64 {
    unsafe { oxidebsd_sys_mlock(addr, len) }
}

extern "C" fn handle_munlock(addr: u64, len: u64, _a2: u64, _a3: u64) -> i64 {
    unsafe { oxidebsd_sys_munlock(addr, len) }
}

extern "C" fn handle_mlockall(flags: u64, _a1: u64, _a2: u64, _a3: u64) -> i64 {
    unsafe { oxidebsd_sys_mlockall(flags) }
}

extern "C" fn handle_munlockall(_a0: u64, _a1: u64, _a2: u64, _a3: u64) -> i64 {
    unsafe { oxidebsd_sys_munlockall() }
}
...
pub extern "C" fn module_init() -> i32 {
    unsafe {
        ...
        oxidebsd_register_syscall(SYS_FUTEX_REQUEUE, handle_futex_requeue);
        oxidebsd_register_syscall(SYS_MLOCK, handle_mlock);
        oxidebsd_register_syscall(SYS_MUNLOCK, handle_munlock);
        oxidebsd_register_syscall(SYS_MLOCKALL, handle_mlockall);
        oxidebsd_register_syscall(SYS_MUNLOCKALL, handle_munlockall);
