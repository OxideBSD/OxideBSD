...
//! is narrower than it used to be.** `IA32_SFMASK` (see `sys/syscall.rs::init`) clears `IF` on
...
/// Real musl x86_64 `struct termios` layout (`external/mit/musl`'s `arch/generic/bits/termios.h`) —
...
/// line-holdback (backspace/erase has always been a userland concern — see `regress/stsh/`'s own
...
/// Real `c_cc[]` indices (`external/mit/musl`'s `arch/generic/bits/termios.h`, the one every
