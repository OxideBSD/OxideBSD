...
/// Also defines `POSIX_TEST_EXTRA_FILES` -- a second, separate array for the one pilot fixture
/// (`sigaltstack/9-1.c`'s own `9-buildonly.test` helper) that needs seeding at a literal path
/// *outside* `/posix-tests` entirely -- see that array's own generation comment in build.rs.
