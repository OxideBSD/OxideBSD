//! Real-`SYSCALL` smoke test for `SYS_SYSINFO = 527` (`sys/modules/posix_compat`'s `handle_sysinfo`
//! -> `sys/syscall/ffi.rs`'s `sys_sysinfo`/`RawSysinfo`) -- item 2 of
...
//! shared crate exists across this ABI boundary, same convention every other regress/kernel pair
//! here uses.
//!
