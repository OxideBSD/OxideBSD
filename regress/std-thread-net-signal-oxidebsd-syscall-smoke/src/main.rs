//! Real-`SYSCALL` smoke test extending v0.3.0's real `std` consumer proof past
//! `std-process-fs-oxidebsd` (fs + `process::Command`): confirms `/bin/std-thread-net-signal-
//! oxidebsd` -- real `std::thread` (spawn/join + `Arc<Mutex<_>>` over real futex(2)), real signal
//! handling (SIGPIPE-ignored-at-startup + real `SIGKILL`/`ExitStatusExt::signal()`), and real
//! `std::net` (UDP/TCP socket()/bind()/listen()/nonblocking accept()) -- actually works against
//! OxideBSD's native syscall ABI.
//!
//! Deliberately a real spawned ELF driven through genuine `SYSCALL`/`SYSRETQ`, not a plain Rust
//! function call from a test's own `main()` -- same reasoning `std-process-fs-oxidebsd-
//! syscall-smoke` documents. `fork` + `execve` `/bin/std-thread-net-signal-oxidebsd`, `wait4` for
//! it, and check the real `wait(2)`-encoded exit status decodes to `42`
//! (`std::process::exit(42)` in that crate's own `main()`, reached only after every internal
//! `assert!`/`.expect()` passes) via `WEXITSTATUS`.
#![no_std]
#![no_main]
...
/// std_thread_net_signal_oxidebsd_syscall_smoke.rs` registers this one directly against a
/// test-only handler, same convention every other real-`SYSCALL` smoke test in this codebase
/// uses.
...
macro_rules! check {
    ($cond:expr, $msg:expr) => {
        if !$cond {
            write_bytes(b"std-thread-net-signal-oxidebsd-syscall-smoke: FAIL: ");
            write_bytes($msg);
            write_bytes(b"\n");
            test_exit(false);
        }
...
pub extern "C" fn _start() -> ! {
    write_bytes(b"std-thread-net-signal-oxidebsd-syscall-smoke: starting\n");
    ...
    match fork() {
        ...
        Ok(child_pid) => {
            ...
            if exit_code != 42 {
                write_bytes(
                    b"std-thread-net-signal-oxidebsd-syscall-smoke: FAIL: unexpected exit code (wanted 42)\n",
                );
                test_exit(false);
            }
            write_bytes(
                b"std-thread-net-signal-oxidebsd-syscall-smoke: /bin/std-thread-net-signal-oxidebsd ran and exited 42 as expected\n",
            );
            test_exit(true);
        }
        Err(_) => {
            write_bytes(b"std-thread-net-signal-oxidebsd-syscall-smoke: fork failed\n");
            test_exit(false);
        }
    }
}
