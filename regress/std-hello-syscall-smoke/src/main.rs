//! Real-`SYSCALL` smoke test for the Rust `std` target Phase-0 spike (see `regress/std/
//! std-hello/main.rs`'s own doc comment, and `build.rs`'s `build_std_hello_spike`): confirms
//! `/bin/std-hello` -- a genuinely unmodified, prebuilt upstream `std` binary for the real
//! `x86_64-unknown-linux-musl` target, statically linked against this project's own patched musl
//! sysroot instead of `rust-lang/rust`'s bundled one -- actually runs against OxideBSD's native
...
//! `WEXITSTATUS` (`(status >> 8) & 0xff` -- see `sys/process/lifecycle.rs`'s own doc comment on
//! this encoding).
#![no_std]
#![no_main]
...
/// std_hello_syscall_smoke.rs` registers this one directly against a test-only handler, same
...
macro_rules! check {
    ($cond:expr, $msg:expr) => {
        if !$cond {
            write_bytes(b"std-hello-syscall-smoke: FAIL: ");
            write_bytes($msg);
            write_bytes(b"\n");
            test_exit(false);
        }
...
pub extern "C" fn _start() -> ! {
    write_bytes(b"std-hello-syscall-smoke: starting\n");
    ...
    match fork() {
        ...
        Ok(child_pid) => {
            ...
            if exit_code != 42 {
                write_bytes(b"std-hello-syscall-smoke: FAIL: unexpected exit code (wanted 42)\n");
                test_exit(false);
            }
            write_bytes(b"std-hello-syscall-smoke: /bin/std-hello ran and exited 42 as expected\n");
            test_exit(true);
        }
        Err(_) => {
            write_bytes(b"std-hello-syscall-smoke: fork failed\n");
            test_exit(false);
        }
    }
}
