//! Real-`SYSCALL` smoke test for `SA_SIGINFO` handler invocation (`sys/syscall/mod.rs`'s
...
//! `external/mit/musl/src/signal/x86_64/restore.s`) doesn't exist here -- `sigreturn_trampoline`
...
/// Real, unclaimed Linux `__NR_tkill` -- see `sys/syscall/ffi.rs`'s `sys_tkill` doc comment.
...
/// Real Linux/x86_64 value -- matches `sys/process/mod.rs`'s own `SA_SIGINFO`.
const SA_SIGINFO: u64 = 0x00000004;
/// Real Linux `SI_USER` -- matches `sys/syscall/mod.rs`'s own `SI_USER`.
const SI_USER: i32 = 0;
...
/// Matches `sys/syscall/mod.rs`'s own `RawSiginfo` byte-for-byte -- see that struct's own doc
...
/// Matches `sys/syscall/mod.rs`'s own `RawUcontext`/`RawMcontext` byte-for-byte.
#[repr(C)]
struct RawMcontext {
    gregs: [i64; 23],
    fpregs: u64,
...
/// `deliver_pending_signal`'s own doc comment in `sys/syscall/mod.rs`).
global_asm!(
