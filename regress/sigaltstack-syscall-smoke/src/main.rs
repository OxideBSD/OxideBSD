//! Real-`SYSCALL` smoke test for `SYS_SIGALTSTACK = 528` (`sys/modules/signal`'s `handle_sigaltstack`
//! -> `sys/syscall/ffi.rs`'s `sys_sigaltstack` -> `sys/process/signals.rs`'s `do_sigaltstack`/
...
/// Real `sigaltstack(2)` flag bits (`external/mit/musl/include/signal.h`).
