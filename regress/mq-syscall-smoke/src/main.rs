...
//! (`sys/modules/posix_compat`'s `handle_mq_*` -> `sys/syscall/ffi.rs`'s `sys_mq_*` ->
//! `sys/fs/mqueue.rs`'s `do_mq_*`) -- items 11-16 of `OxideBSD-doc/MISSING_POSIX_SYSCALLS.md`'s own
...
//! `regress/pause-syscall-smoke/` already established). `mqd_and_len` packing (see
//! `sys/fs/mqueue.rs`'s own doc comment) is done by hand here too, mirroring
//! `external/mit/musl/src/mq/mq_timedsend.c`'s own patch.
//!
...
/// Matches the kernel's own `RawMqAttr` (`sys/fs/mqueue.rs`) exactly -- four `long`s plus four
...
/// A real `struct sigevent` shape (`external/mit/musl/include/signal.h`) -- only `sigev_value`/
