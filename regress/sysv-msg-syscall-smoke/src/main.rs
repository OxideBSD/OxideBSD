...
//! (`sys/modules/posix_compat`'s `handle_msg*` -> `sys/syscall/ffi.rs`'s `sys_msg*` ->
//! `sys/fs/sysv_msg.rs`'s `do_msg*`) -- items 25-28 of `OxideBSD-doc/MISSING_POSIX_SYSCALLS.md`'s own
...
//! `syscall()` helper (same convention `regress/mq-syscall-smoke/` already established).
//! `q_and_flag` packing for `msgrcv` (see `sys/fs/sysv_msg.rs`'s own doc comment) is done by hand
//! here too, mirroring `external/mit/musl/src/ipc/msgrcv.c`'s own patch.
//!
...
/// in on this build (see `sys/fs/sysv_msg.rs`'s own doc comment); a raw `syscall()` caller like
...
/// Matches the kernel's own `RawIpcPerm` (`sys/fs/sysv_msg.rs`) exactly.
#[repr(C)]
struct RawIpcPerm {
    key: i32,
    uid: u32,
...
/// Matches the kernel's own `RawMsqidDs` (`sys/fs/sysv_msg.rs`) exactly.
#[repr(C)]
struct RawMsqidDs {
    msg_perm: RawIpcPerm,
    msg_stime: i64,
