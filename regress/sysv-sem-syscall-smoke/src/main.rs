...
//! (`sys/modules/posix_compat`'s `handle_sem*` -> `sys/syscall/ffi.rs`'s `sys_sem*` ->
//! `sys/fs/sysv_sem.rs`'s `do_sem*`) -- items 21-24 of `OxideBSD-doc/MISSING_POSIX_SYSCALLS.md`'s own
...
//! `syscall()` helper (same convention `regress/sysv-msg-syscall-smoke/` already established).
//!
...
/// on this build (see `sys/fs/sysv_sem.rs`'s own doc comment); a raw `syscall()` caller like this
...
/// Matches the kernel's own `RawSembuf` (`sys/fs/sysv_sem.rs`) exactly -- 6 bytes.
#[repr(C)]
#[derive(Clone, Copy)]
struct RawSembuf {
    sem_num: u16,
...
/// Matches the kernel's own `RawIpcPerm` (`sys/fs/sysv_ipc.rs`) exactly.
#[repr(C)]
struct RawIpcPerm {
    key: i32,
    uid: u32,
...
/// Matches the kernel's own `RawSemidDs` (`sys/fs/sysv_sem.rs`) exactly.
#[repr(C)]
struct RawSemidDs {
    sem_perm: RawIpcPerm,
    sem_otime: i64,
