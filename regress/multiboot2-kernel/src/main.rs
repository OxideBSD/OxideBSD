...
//! than reusing `sys/main.rs` directly the way `regress/multiboot2-boot-smoke` reuses
//! `tests/multiboot2_boot_smoke.rs`). `run_real_system` is the exact function the ordinary Limine
//! boot path (`sys/main.rs`'s `kernel_main`) also calls -- same module loading, same `hush` spawn,
