...
//! and `sys/process/mm.rs`/`sys/cpu/interrupts.rs`/`sys/process/fault_trampoline.rs` for the real
...
//! the same fork-and-check-wait4-status shape `regress/sig-syscall-smoke`'s own permission-check
...
//! `regress/pause-syscall-smoke/` already established) -- part 2's handler never actually
...
/// `sys/modules/oxfs`'s own `oxfs_open` enforces the caller's real requested access mode even on a
...
/// Real `mmap(2)` flag bits, matching `external/mit/musl/include/sys/mman.h` -- now real on the
...
/// Matches `sys/process/limits.rs`'s own `RawRlimit` wire format exactly (`(rlim_cur, rlim_max)`).
#[repr(C)]
struct RawRlimit {
    rlim_cur: u64,
    rlim_max: u64,
...
/// Matches `sys/modules/oxfs`'s own `MuslStat` wire format exactly (`external/mit/musl`'s real x86_64
...
/// Matches `sys/process/timers.rs`'s own `RawTimespec`/real `struct timespec` wire format exactly.
#[repr(C)]
struct RawTimespec {
    tv_sec: i64,
    tv_nsec: i64,
...
/// `sys/syscall/ffi.rs`'s own `oxidebsd_sys_mmap` doc comment for this exact wire format. `packed`
/// = `(off << 32) | (fd as u32)`.
