...
//! `sys/process/signals.rs`'s `record_pending`/`take_deliverable_signal`/`do_sigtimedwait` RT
...
//! see `sys/syscall/mod.rs`'s `deliver_pending_signal`/`do_sigreturn`), landed later; parts 1/3
...
//! `tests/posix_conformance_smoke.rs` + `regress/posix-conformance-driver/`, not this crate), it
...
//! and minimal `sigreturn_trampoline`, same convention `regress/sig-syscall-smoke/` established.
//!
...
/// Real, unremapped Linux `__NR_sigreturn` slot -- see `external/mit/musl/src/signal/x86_64/
...
/// Real `sigprocmask(2)` `how` values -- matches `sys/process/signals.rs`'s own `do_sigprocmask`.
const SIG_BLOCK: u64 = 0;
const SIG_UNBLOCK: u64 = 1;

/// Must match `sys/process/mod.rs`'s own `SIGRTMIN`/`SIGRTMAX` exactly (musl's `sigrtmin.c`
/// hardcodes `35`; `SIGRTMAX = _NSIG - 1 = 64`).
const SIGRTMIN: u64 = 35;
const SIGRTMAX: u64 = 64;
/// Must match `sys/process/signals.rs`'s own `RT_QUEUE_CAP`.
...
/// Matches `sys/process/mod.rs`'s own `RawSiginfo` wire format exactly.
#[repr(C)]
struct RawSiginfo {
    si_signo: i32,
    si_errno: i32,
...
/// Matches `sys/process/signals.rs`'s `resolve_relative_deadline`'s own `(sec, nsec)` pair.
#[repr(C)]
struct RawTimespec {
    sec: i64,
    nsec: i64,
...
/// This crate's own minimal `__restore_rt` equivalent -- see `regress/pause-syscall-smoke/`'s own
...
/// `regress/sa-siginfo-syscall-smoke/`'s own already-proven signature exactly.
...
pub extern "C" fn _start() -> ! {
    ...
    // see `deliver_pending_signal`/`do_sigreturn` in `sys/syscall/mod.rs`.
    check!(
