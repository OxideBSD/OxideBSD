...
//! (`sys/modules/signal`'s `handle_sigtimedwait`/`handle_sigqueue` -> `sys/syscall/ffi.rs`'s
//! `sys_sigtimedwait`/`sys_sigqueue` -> `sys/process/signals.rs`'s `do_sigtimedwait`/
...
const EINVAL: u64 = 22;
const EAGAIN: u64 = 11;
/// Real value, matches `sys/syscall/mod.rs`'s own `EPERM` -- identical on Linux/BSD/musl.
const EPERM: u64 = 1;
const ESRCH: u64 = 3;
...
/// Real Linux `si_code` values (`external/mit/musl/include/bits/siginfo.h`) -- matches
/// `sys/process/mod.rs`'s own `SI_USER`/`SI_QUEUE` exactly.
const SI_USER: i32 = 0;
...
/// Matches `sys/process/signals.rs`'s own `RawTimespec`-shaped `(sec, nsec)` pair every
