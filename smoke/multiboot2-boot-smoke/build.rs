...
//! crate's own binary -- same rationale as every `userland/*` crate's own `build.rs` (see e.g.
//! `userland/fork-exec-smoke/build.rs`): a `RUSTFLAGS` override would apply uniformly to every
...
//! different flags.
//!
//! Unlike a `userland/*` crate's own same-directory `linker.ld`, this one lives at the repo root
