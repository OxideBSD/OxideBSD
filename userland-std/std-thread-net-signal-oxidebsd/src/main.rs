...
/// Matches `src/net/ipv4.rs`'s own `GUEST_IP` -- the one real address this kernel's single NIC
...
fn test_net() {
    ...
    // exists on this kernel (single real NIC, see src/net/ipv4.rs's own doc comment) -- this
    ...
    // (fcntl) + a real EAGAIN-returning accept() path (src/net/tcp.rs's oxidebsd_sys_accept).
