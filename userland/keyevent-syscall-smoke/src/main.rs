//! Real-`SYSCALL` smoke test for `SYS_GET_KEYEVENT = 558` (`modules/posix_compat`'s
//! `handle_get_keyevent` -> `src/syscall/ffi.rs`'s `sys_get_keyevent` -> `console::keyevents`'s
...
/// Must match `src/console/keyevents.rs`'s own `RawKeyEvent` exactly -- no shared crate across
/// this ABI boundary, same convention every other userland/kernel wire-struct pair here uses.
#[repr(C)]
