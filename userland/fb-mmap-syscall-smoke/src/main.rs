...
//! own plan notes): a real `/dev/fb0` device (`modules/oxfs`'s `known_device`'s `(29, 0)` arm), a
...
/// Must match `src/syscall/ffi.rs`'s own `FBIOGET_OXIDEBSD`.
const FBIOGET_OXIDEBSD: u64 = 0x4600;

const PROT_READ_WRITE: u64 = 0x1 | 0x2;
/// Must match `src/process/mm.rs`'s own `MAP_SHARED`.
const MAP_SHARED: u64 = 0x01;

/// Must match `src/syscall/ffi.rs`'s own `RawFbInfo` exactly -- no shared crate across this ABI
/// boundary, same convention every other userland/kernel wire-struct pair here uses.
#[repr(C)]
