//! Real-`SYSCALL` smoke test for v0.3.0's "first real `std` consumer" proof: confirms
//! `/bin/std-process-fs-oxidebsd` -- real `std::fs` (write/read_to_string/remove_file) and real
//! `std::process::Command` (its own internal fork+execve+waitpid, spawning `/bin/true`/
//! `/bin/echo`) -- actually works against OxideBSD's native syscall ABI, not just the
//! `println!`/`process::exit` runtime-startup/shutdown path `std-hello-oxidebsd` already covers.
//!
//! Deliberately a real spawned ELF driven through genuine `SYSCALL`/`SYSRETQ`, not a plain Rust
//! function call from a test's own `main()` -- same reasoning `std-hello-oxidebsd-syscall-smoke`
//! documents. `fork` + `execve` `/bin/std-process-fs-oxidebsd`, `wait4` for it, and check the real
//! `wait(2)`-encoded exit status decodes to `42` (`std::process::exit(42)` in
//! `std-process-fs-oxidebsd`'s own `main()`, reached only after every internal `assert!`/
//! `.expect()` passes) via `WEXITSTATUS`.
#![no_std]
#![no_main]
...
/// std_process_fs_oxidebsd_syscall_smoke.rs` registers this one directly against a test-only
/// handler, same convention every other real-`SYSCALL` smoke test in this codebase uses.
...
macro_rules! check {
    ($cond:expr, $msg:expr) => {
        if !$cond {
            write_bytes(b"std-process-fs-oxidebsd-syscall-smoke: FAIL: ");
            write_bytes($msg);
            write_bytes(b"\n");
            test_exit(false);
        }
...
pub extern "C" fn _start() -> ! {
    write_bytes(b"std-process-fs-oxidebsd-syscall-smoke: starting\n");
    ...
    match fork() {
        ...
        Ok(child_pid) => {
            ...
            if exit_code != 42 {
                write_bytes(
                    b"std-process-fs-oxidebsd-syscall-smoke: FAIL: unexpected exit code (wanted 42)\n",
                );
                test_exit(false);
            }
            write_bytes(
                b"std-process-fs-oxidebsd-syscall-smoke: /bin/std-process-fs-oxidebsd ran and exited 42 as expected\n",
            );
            test_exit(true);
        }
        Err(_) => {
            write_bytes(b"std-process-fs-oxidebsd-syscall-smoke: fork failed\n");
            test_exit(false);
        }
    }
}
