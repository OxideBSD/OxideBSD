...
# RCSid:
#	$Id: os.sh,v 1.71 2026/08/23 19:50:32 sjg Exp $
#
...
Echo() {
	eval $_local _echo_flags _echo_c
	_echo_flags=
	_echo_c=
	while :
	do
		case "$1" in
		-e) shift; _echo_flags="$_echo_flags -e";;
		-n) shift; _echo_flags="$_echo_flags -n" _echo_c="$echo_c";;
		*) break;;
		esac
	done
	echo $_echo_flags "$@""$_echo_c"
}
